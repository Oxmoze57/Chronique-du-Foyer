...
async function addHouseholdIdeasToOath(){
  ...
  const selected=$$('#oathPicker input:checked').map(x=>x.value).slice(0,3);
  const slots=Math.max(0,3-selected.length);
  if(!slots){if(status)status.textContent='Ton Serment contient déjà 3 missions. Décoche-en une pour demander une idée ménagère.';return}
  ...
  try{
    ...
    const wanted=new Set([...selected,...added.map(t=>t.id)].slice(0,3));
...
function freshState(){return{schemaVersion:6,tasks:defaultTasks,projects:defaultProjects,xp:0,shards:0,activeDays:[],oath:[],oathDate:dateKey(),oathsByDate:{},pathFilter:'all',virtues:{Foyer:0,Artisanat:0,Soutien:0,Discipline:0},ritualTime:'21:30',lastRitualNotice:'',week:{key:weekKey(),count:0},achievements:[],stats:{missions:0,minutes:0,oaths:0,rescues:0}}}
...
function completeTask(t){const d=dateKey();t.doneDates=t.doneDates||[];if(t.doneDates.includes(d))return;const beforeOath=oathComplete();t.doneDates.push(d);if(t.type!=='routine')t.completedAt=d;const xp=xpFor(t);state.xp+=xp;state.shards+=shardFor(t);state.stats.missions++;state.stats.minutes+=Number(t.duration||0);state.week.count++;const v=pathVirtues[t.path]||'Foyer';state.virtues[v]=(state.virtues[v]||0)+1;if(!state.activeDays.includes(d))state.activeDays.push(d);if(!beforeOath&&oathComplete()&&getOath().length>=1){state.xp+=75;state.shards+=5;state.stats.oaths++;setTimeout(()=>showReward('Serment tenu !','+75 XP bonus','Tu as tenu tes engagements essentiels. Le reste est du bonus.',{rank:'S',xp:75,mission:'Serment du jour'}),80)}else{showReward('Mission accomplie',`+${xp} XP · +${shardFor(t)} éclat${shardFor(t)>1?'s':''}`,t.path==='Soutien'?'Tu viens directement d’alléger la charge du foyer.':'Une action concrète de plus. Le foyer avance.',{task:t,xp})}unlockAchievements();confetti()}
function oathComplete(){const items=getOath().map(id=>state.tasks.find(t=>t.id===id)).filter(Boolean);return items.length>0&&items.every(isDone)}
function renderOath(){const box=$('#oathList');box.innerHTML='';const items=getOath().map(id=>state.tasks.find(t=>t.id===id)).filter(Boolean).slice(0,3);const done=items.filter(isDone).length;$('#oathDone').textContent=done;$('#oathProgress').style.width=`${items.length?done/items.length*100:0}%`;if(!items.length){box.innerHTML='<div class="empty">Choisis 1 à 3 engagements essentiels. Peu, mais tenus.</div>';return}items.forEach((t,i)=>{const row=document.createElement('div');row.className=`oath-item ${isDone(t)?'done':''}`;row.innerHTML=`<span class="oath-index">${i+1}</span><div><strong>${esc(t.title)}</strong><div class="task-meta">${pathIcons[t.path]||'🛡️'} ${t.duration} min</div></div><span>${isDone(t)?'✓':'→'}</span>`;row.onclick=()=>openFocus(t);box.appendChild(row)})}
...
function openOathPicker(tomorrow=false,energy='normal'){
  ...
  pool.forEach((t,i)=>{const suggested=tomorrow&&saved.size===0&&i<3&&t.priority!=='bonus';const row=document.createElement('label');row.className='oath-item';row.innerHTML=`<input type="checkbox" value="${t.id}" ${(saved.has(t.id)||suggested)?'checked':''}><div><strong>${esc(t.title)}</strong><div class="task-meta">${pathIcons[t.path]||'🛡️'} ${t.path}${t.room?` · 📍 ${esc(t.room)}`:''} · ${t.duration} min · ${priorityLabel(t.priority)}</div></div><span>+${xpFor(t)} XP</span>`;const input=row.querySelector('input');input.onchange=()=>{const checked=$$('#oathPicker input:checked');if(checked.length>3){input.checked=false;const status=$('#oathSuggestionStatus');if(status)status.textContent='Un Serment contient au maximum 3 missions. Peu, mais tenues.'}};picker.appendChild(row)});
...
$('#doneFocus').onclick=()=>{const rescue=isRescueFocus;if(focusCompletion){const fn=focusCompletion;fn();if(rescue)state.stats.rescues++;save();closeFocus();render();return}if(currentFocus?.id==='rescue'){state.xp+=10;state.shards+=1;state.stats.missions++;state.stats.minutes+=5;state.stats.rescues++;if(!state.activeDays.includes(dateKey()))state.activeDays.push(dateKey());save();showReward('Élan retrouvé','+10 XP · +1 éclat','Cinq minutes suffisent parfois à changer la direction d’une soirée.',{rank:'C',xp:10,mission:'Mission de reprise'});confetti();closeFocus();render();return}if(currentFocus&&state.tasks.some(t=>t.id===currentFocus.id)){const real=state.tasks.find(t=>t.id===currentFocus.id);if(!isDone(real))completeTask(real);if(rescue)state.stats.rescues++;save()}closeFocus();render()};
$('#closeFocus').onclick=closeFocus;
$('#editOathBtn').onclick=()=>openOathPicker(false);
$('#closeOath').onclick=()=>$('#oathDialog').close();
$('#saveOath').onclick=()=>{const d=$('#oathPicker').dataset.forDate||dateKey();setOath(d,$$('#oathPicker input:checked').map(x=>x.value).slice(0,3));save();$('#oathDialog').close();render()};
